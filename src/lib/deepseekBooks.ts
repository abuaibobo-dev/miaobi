...
const CATEGORIES: ContentCategory[] = ['all', 'book', 'magazine', 'newspaper', 'story', 'art', 'adult'];
...
export async function parseBookQuery(query: string): Promise<ParsedBookQuery> {
  ...
  const content = await callDeepSeek([
    { role: 'system', content: '你是找书检索规划器。把用户自然语言转换成用于公开书目API的JSON。只输出JSON：{"queries":["..."],"language":"zh|en","category":"all|book|magazine|newspaper|story|art|adult","intent":["..."]}。如果用户输入作者名，queries必须包含作者原名和常见英文/中文译名；如果是作品描述，补充可能的书名、作者、题材关键词。queries包含1-4个精炼搜索词。' },
...
export async function suggestDiscoveries(): Promise<Array<{ title: string; query: string; category: string }>> {
  const content = await callDeepSeek([
    { role: 'system', content: '你是书源发现编辑。返回JSON：{"items":[{"title":"短标题","query":"适合公开书源的搜索词","category":"book|story|magazine|newspaper|art|adult"}]}，共8项。不要重复；覆盖小说、故事、杂志、报纸、艺术图片。' },
